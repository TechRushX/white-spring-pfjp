...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['pfjp', 'usp', 'nfp', 'cps', 'wpb', 'ind'];
    }
    if (!partyColors) {
        partyColors = {'pfjp': '#3f7bc1', 'usp': '#800080', 'nfp': '#000000', 'cps': '#E3000F', 'wpb': '#DCCA4A', 'ind': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'pfjp': 'PFJP', 'usp': 'USP', 'nfp': 'NFP', 'cps': 'CPS', 'wpb': 'WPB', 'ind': 'Independents'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      if (!dataMax) {
            dataMax = d3.max(data, d => Math.max(d.pfjp, d.usp, d.nfp, d.cps, d.wpb, d.ind));
            dataMin = 0;
      }
